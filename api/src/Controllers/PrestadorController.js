...
module.exports = {
  ...
  async getPrestadorById(req, res) {
    try {
      ...
      // Calcular estadísticas de calificaciones: promedio y cantidad
      try {
        const stats = await Calificacion.findAll({
          where: { id_prestador: id },
          attributes: [
            [fn('AVG', col('estrellas')), 'promedio'],
            [fn('COUNT', col('id_calificacion')), 'cantidad']
          ],
          raw: true
        });
        const row = Array.isArray(stats) && stats.length > 0 ? stats[0] : null;
        const promedio = row && row.promedio ? parseFloat(row.promedio) : 0;
        const cantidad = row && row.cantidad ? parseInt(row.cantidad) : 0;
        // anexar al objeto devuelto
        prestador.dataValues = prestador.dataValues || {};
        prestador.dataValues.calificacionPromedio = Number(promedio.toFixed ? promedio.toFixed(1) : promedio) || 0;
        prestador.dataValues.cantidadCalificaciones = cantidad || 0;
      } catch (eStats) {
        console.warn('No se pudieron calcular estadísticas de calificaciones:', eStats);
      }
  ...
  async updatePrestador(req, res) {
    try {
      ...
      if (
        descripcion &&
        !validators.isValidLength(descripcion.trim(), 0, 1000)
      ) {
        validationErrors.push({
          field: "descripcion",
      ...
      if (
        experiencia &&
        !validators.isValidLength(experiencia.trim(), 0, 1000)
      ) {
        validationErrors.push({
          field: "experiencia",
      ...
      const updateData = {};
      if (nombre_completo)
        updateData.nombre_completo = validators.sanitizeString(nombre_completo);
      ...
      if (descripcion !== undefined)
        updateData.descripcion = descripcion
          ? validators.sanitizeString(descripcion)
          : null;
      if (experiencia !== undefined)
        updateData.experiencia = experiencia
          ? validators.sanitizeString(experiencia)
          : null;
  ...
  async saveImage(req, res) {
    try {
      ...
      return ResponseService.created(
        res,
        newImage,
        "Imagen subida y guardada exitosamente"
      );
    } catch (error) {
        ...
        include: [
          {
            ...
            include: [
              ...
              },
              {
                ...
                as: "presupuestos",
                where: { id_prestador: id },
                required: false, // por si aún no hay presupuesto
              },
            ],
          },
        ],
  ...
  async updatePrestadorImage(req, res) {
    try {
      ...
      return ResponseService.serverError(
        res,
        "Error interno al actualizar la imagen"
      );
    }
  },
  ...
  async getPrestadorImages(req, res) {
    try {
      ...
      return ResponseService.error(
        res,
        "Error interno al obtener las imágenes",
        HTTP_STATUS.INTERNAL_SERVER_ERROR
      );
    }
  },
  async getPresupuestosEnviados(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ error: "Error al obtener los presupuestos enviados" });
    }
  },
  ...
  async rejectSolicitud(req, res) {
    try {
      ...
      if (!id_prestador) {
        return res
          .status(403)
          .json({ error: "No se pudo determinar el prestador autenticado" });
      }

      const solicitudPrestador = await SolicitudPrestador.findOne({
        where: { id_solicitud_prestador: id, id_prestador },
      });

      if (!solicitudPrestador) {
        return res
          .status(404)
          .json({ error: "Solicitud del prestador no encontrada" });
      }
      ...
      if (message) {
        try {
          const solicitudServicio = await SolicitudServicio.findByPk(solicitudPrestador.id_solicitud);
          ...
          // obtener nombre del prestador para el texto
          let prestadorNombre = null;
          try {
            const prestadorRecord = await Prestador.findByPk(id_prestador);
            prestadorNombre = prestadorRecord?.nombre_completo || null;
          } catch (pErr) {
            console.warn('No se pudo obtener nombre del prestador para notificación', pErr);
          }

          // construir mensaje final: incluir nombre del prestador y entre comillas el mensaje opcional
          const mensajeFinal = prestadorNombre
            ? `El prestador ${prestadorNombre} rechazó el presupuesto.${message ? ` "${message}"` : ''}`
            : `El prestador rechazó el presupuesto.${message ? ` "${message}"` : ''}`;

          if (idUsuarioDestino) {
            await createAutomaticNotification(idUsuarioDestino, 'in-app', mensajeFinal, solicitudPrestador.id_solicitud);
          } else {
            // fallback: crear notificacion local si no encontramos cliente
          ...
          console.warn('Error creando notificación al rechazar solicitud del prestador', notifErr);
        }
      }

      return res
        .status(200)
        .json({ message: "Solicitud del prestador rechazada correctamente" });
    } catch (error) {
      console.error("Error al rechazar la solicitud del prestador:", error);
      return res
        .status(500)
        .json({ error: "Error al rechazar la solicitud del prestador" });
    }
  },
};
